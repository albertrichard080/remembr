...
class VILACaptionerWithHistory(VILACaptioner):
    def __init__(self, args):
        super().__init__(args)
        self.history = []  # Store previous images and prompts
    
    def append_to_history(self, image, prompt):
        """Append image and prompt to history for context."""
        self.history.append((image, prompt))
    
    def reset_history(self):
        """Reset the history to avoid excessive memory usage."""
        self.history = []
    
    def generate_caption_with_history(self, images, prompt):
        """Generate a caption using the history of images and prompts."""
        # Add the current image and prompt to the history
        self.append_to_history(images, prompt)
        
        # Combine all previous history (images and prompts) for context
        combined_images = [img for img, _ in self.history]
        combined_prompts = [p for _, p in self.history]
        
        # Generate the caption based on combined history
        caption = self.generate_caption(combined_images, combined_prompts)
...
class CaptionerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            + "features, events/activities, and other interesting details. Think step "
        ...
        # Pass args to VILACaptionerWithHistory
        if not self.debug:
            self.model = VILACaptionerWithHistory(args)  # Now using VILACaptionerWithHistory
    ...
    def caption_images(self, images):
        """Generate a caption for a list of images using VILACaptionerWithHistory."""
        if self.debug:
            return "Dummy caption"

        try:
            # Call VILACaptionerWithHistory's method to generate caption with history
            caption = self.model.caption(images, self.prompt)
            return caption
        except Exception as e:
